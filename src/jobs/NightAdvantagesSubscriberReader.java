...
public class NightAdvantagesSubscriberReader extends JdbcCursorItemReader {
	...
	public NightAdvantagesSubscriberReader() {
		...
		setSql("SELECT ID,SUBSCRIBER,FLAG,CREATED_DATE_TIME,ORIGIN_OPERATOR_ID FROM " + tableName + " WHERE ((CREATED_DATE_TIME_INDEX = " + Integer.parseInt((new SimpleDateFormat("yyyyMMdd")).format(now)) + ") AND (FLAG = 1) AND (SMS IS NULL))");
	}
